...
// Editted by gurdeep 6/12/25
//

#ifndef INTEGRATORS_ROTATIONAL_BINARY_SYSTEM_NEIGHBORS_PBC_OMP_H
#define INTEGRATORS_ROTATIONAL_BINARY_SYSTEM_NEIGHBORS_PBC_OMP_H
...
class rotational_binary_system_neighbors_pbc_omp : public rotational_generic_system<field_value_t, real_t, integrator_t, step_handler_t,
        rotational_binary_system_neighbors_pbc_omp<field_value_t, real_t, integrator_t, step_handler_t, acceleration_handler_t, have_unary_force>> {
public:
    typedef std::vector<field_value_t> field_container_t;
    typedef std::vector<long> index_container_t;

    rotational_binary_system_neighbors_pbc_omp(rotational_binary_system_neighbors_pbc_omp const &) = delete;
    ...
    rotational_binary_system_neighbors_pbc_omp(long n_part,
                                real_t r_verlet,
    ...
    // Call the superclass constructor
            rotational_generic_system<field_value_t, real_t, integrator_t, step_handler_t, rotational_binary_system_neighbors_pbc_omp>(std::move(x0),
...
#pragma omp parallel for default(none) shared(t)
    ...
    void update_neighbor_list(std::vector<real_t>& r,
                              const BoxType& box)
    {
    #pragma omp parallel for default(none) shared(r, box)
        for (long i = 0; i < n_part; i++)
        {
            ...
            for (long j = 0; j < n_part; j++)
            {
                ...
                // proper triclinic minimum image
                d = box.minimumImage(d);

                real_t distance = d.norm() - (r[i] + r[j]);
...
#endif //INTEGRATORS_ROTATIONAL_BINARY_SYSTEM_NEIGHBORS_PBC_OMP_H
